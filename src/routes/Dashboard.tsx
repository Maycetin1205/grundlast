...
export default function Dashboard() {
  ...
  const gaps = ap1Coverage(progress).filter((item) => !item.contentVerified)
  return <article className="v2-page">
    ...
    <section className="v2-section"><div className="v2-section-head"><h2>AP1-Inhaltsprüfung</h2><span className="v2-muted">{gaps.length} Kompetenzen noch nicht vollständig belegt</span></div>{gaps.slice(0, 3).map((gap) => <div className="v2-topic-row" key={gap.id}><span><span className="v2-row-title">{gap.titel}</span><span className="v2-row-sub">{gap.verified} von {gap.total} zugeordneten Kapiteln fachlich geprüft</span></span></div>)}</section>
  </article>
}
