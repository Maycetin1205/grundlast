...
import { chapterNeighbors, chapterTrust, getChapter, isChapterAvailable, loadChapter, useProgress } from '../lib/learning'
...
export default function Lesson() {
  ...
  const sources = chapter ? getSourcesForLesson(chapter.slug) : []
  ...
  return <article className="lesson-shell"><div className="lesson-topbar"><span>LF {chapter.primaryLf} / {chapter.titel}</span><span>{nav.previous && <Link className="v2-secondary !h-[30px] !px-2" to={nav.previous.to} aria-label="Vorheriges Kapitel"><ChevronLeft size={14}/></Link>} {nav.next && <Link className="v2-secondary !h-[30px] !px-2" to={nav.next.to} aria-label="Nächstes Kapitel"><ChevronRight size={14}/></Link>}</span></div><div className="lesson-grid"><main className="lesson-article"><header><div className="v2-nav-label !m-0 !p-0 !text-[var(--acc-text)]">LF {chapter.primaryLf}</div><h1>{chapter.titel}</h1><div className="v2-path-meta"><span>{chapter.lernzeit} Minuten</span><Link to="#quellen">{sources.length} belegte Quellen</Link><span className={`v2-trust-badge ${chapter.inhaltsstatus}`}>{trust.shortLabel}</span><select aria-label="Persönliche Selbsteinschätzung" value={progress[chapter.slug] ?? 'not_started'} onChange={(event) => setProgress(chapter.slug, event.target.value as 'not_started'|'learning'|'review'|'mastered')}><option value="not_started">Nicht begonnen</option><option value="learning">In Arbeit</option><option value="review">Wiederholen</option><option value="mastered">Selbst eingeschätzt: sicher</option></select></div></header><div className={`v2-trust-banner ${chapter.inhaltsstatus}`} role="status"><strong>{trust.label}</strong><span>{trust.explanation}</span></div><GrundlastMDXProvider><div className="lesson-content"><Content/><section id="quellen"><h2>Quellen</h2>{sources.length ? <LessonSources sources={sources}/> : <p>Für dieses Kapitel sind im zentralen Katalog noch keine belastbaren Quellen hinterlegt. Der Text gilt deshalb nicht als fachlich freigegeben.</p>}</section><ChapterFooter previous={nav.previous} next={nav.next}/></div></GrundlastMDXProvider></main><aside className="lesson-toc"><Link className="v2-secondary" to="/">Zur Startseite</Link></aside></div></article>
}
