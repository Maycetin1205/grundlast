...
> Arbeitsdatei fuer das Lernfeld `daten`.
> Tabellen, Beziehungen, Abfragen.
...
Sobald Dateien mit dem Praefix `daten-` unter `material/` liegen,
...
**0 von 4 Kapiteln** stehen auf `ready` oder `final`.
...
### Modul: ER-Modell & SQL  `er-sql`

| Kapitel | Slug | AP1 | Wicht. | Status | Offen fuer ready |
|---|---|:--:|:--:|---|---|
| Was ist eine Datenbank? & ER-Modell | `er-grundlagen` | ja | 5 | `draft` | Ziele, Analogie, Terme, Schritte, Quellen |
| SQL-Grundlagen (SELECT, JOIN, DML) | `sql-grundlagen` | – | 5 | `draft` | Ziele, Analogie, Terme, Quellen |

### Modul: Datenqualität & -typen  `daten-qualitaet`

| Kapitel | Slug | AP1 | Wicht. | Status | Offen fuer ready |
|---|---|:--:|:--:|---|---|
| Normalisierung (1NF, 2NF, 3NF) | `normalisierung` | ja | 5 | `draft` | Ziele, Analogie, Terme, Quellen |
| Stamm- und Bewegungsdaten | `stamm-bewegungsdaten` | ja | 4 | `stub` | **keine MDX-Datei** |
