# Projekt & Prozesse

> Arbeitsdatei fuer das Lernfeld `projekt`.
> Planen, steuern, dokumentieren.

Ein Chat, der an einem Kapitel dieses Lernfelds arbeitet, liest **diese Datei**
und zusaetzlich `UNIVERSAL_PROMPT.md` sowie `CONTENT_GUIDELINES.md`. Mehr nicht.

---

## Quellenpolitik fuer dieses Lernfeld

<!-- Von Hand ausfuellen. Was zaehlt hier als Primaerquelle, was nicht? -->

**Primaer (belegt Fakten):**

- _noch auszufuellen_

**Sekundaer (Einordnung, nie alleiniger Beleg):**

- _noch auszufuellen_

**Nicht zulaessig als Beleg:**

- _noch auszufuellen_

---

## Aufgabentypen, die die Pruefung hier stellt

<!-- Aus dem Material herleiten, nicht raten. Erst ausfuellen, wenn Pruefungen
     unter material/pruefungen/ liegen. -->

| Aufgabentyp | Wie erkennbar | Worauf es bei der Antwort ankommt |
|---|---|---|
| _noch auszufuellen_ | | |

---

## Typische Fehler in diesem Fach

<!-- Fachspezifisch, nicht allgemein. Was geht in genau diesem Lernfeld schief? -->

- _noch auszufuellen_

---

## Material

<!-- GENERIERT:material — nicht von Hand aendern, wird ueberschrieben -->

Noch kein Material zugeordnet.

Sobald Dateien mit dem Praefix `projekt-` unter `material/` liegen,
erscheinen sie hier automatisch. Namensschema siehe `material/README.md`.

<!-- /GENERIERT:material -->

---

## Kapitelbestand

<!-- GENERIERT:kapitel — nicht von Hand aendern, wird ueberschrieben -->

**9 von 14 Kapiteln** stehen auf `ready` oder `final`.

Spalte "Offen fuer ready" listet die Pflichtbausteine, die der Content-Validator
noch vermisst. Leer beziehungsweise "—" heisst: Kapitel erfuellt den Ready-Standard
mechanisch. Das ersetzt keine fachliche Pruefung.

### Modul: Service & Support  `service-management`

| Kapitel | Slug | AP1 | Wicht. | Status | Offen fuer ready |
|---|---|:--:|:--:|---|---|
| Serviceanfragen, Ticketsystem & Support-Level | `serviceanfragen-support-level` | ja | 5 | `ready` | Terme, Schritte |
| Fehlermanagement & Störungsannahme | `fehlermanagement-stoerungsannahme` | ja | 5 | `ready` | Terme |
| Bedarfsanalyse, Kommunikation & Feedback | `bedarfsanalyse-feedback` | ja | 5 | `ready` | — |
| Mitarbeitermotivation, Teamphasen und Change | `mitarbeitermotivation-teamphasen-change` | ja | 5 | `ready` | Schritte |
| Schulung, Einweisung, Key User & Multiplikatoren | `schulung-einweisung-key-user` | ja | 5 | `ready` | Schritte |
| 4-Seiten-Modell nach Schulz von Thun | `schulz-von-thun` | ja | 5 | `ready` | Terme, Schritte, Fehler |

### Modul: Projektplanung  `projektplanung`

| Kapitel | Slug | AP1 | Wicht. | Status | Offen fuer ready |
|---|---|:--:|:--:|---|---|
| Netzplantechnik (FAZ/FEZ/SAZ/SEZ) | `netzplan` | ja | 5 | `draft` | Ziele, Terme, Schritte, Quellen |
| Gantt-Diagramm — Balkenplan | `gantt-diagramm` | ja | 5 | `ready` | Terme, Schritte, Fehler |
| Vorgehensmodelle (Wasserfall, V-Modell, Agil) | `vorgehensmodelle` | ja | 5 | `ready` | — |
| BPMN – Prozesse modellieren | `bpmn` | ja | 5 | `draft` | Ziele, Terme, Schritte, Quellen |
| Scrum – Accountabilities, Events, Artefakte | `scrum` | ja | 4 | `ready` | — |

### Modul: Projektdokumente & Abschluss  `dokumente`

| Kapitel | Slug | AP1 | Wicht. | Status | Offen fuer ready |
|---|---|:--:|:--:|---|---|
| Lastenheft & Pflichtenheft | `lastenheft-pflichtenheft` | ja | 4 | `draft` | Ziele, Terme, Schritte, Quellen |
| Machbarkeitsanalyse | `machbarkeitsanalyse` | ja | 4 | `stub` | **keine MDX-Datei** |
| Projektübergabe & Abnahme | `projektubergabe` | – | 3 | `stub` | **keine MDX-Datei** |

<!-- /GENERIERT:kapitel -->

---

## Reihenfolge und Voraussetzungen

<!-- In welcher Reihenfolge sollte gelernt werden, und was setzt was voraus?
     Das ist die Frage, die eine flache Kapitelliste nicht beantwortet. -->

_noch auszufuellen_

---

## Offene Punkte

<!-- Widersprueche zwischen Material und Primaerquelle, ungeklaerte Zuordnungen,
     Kapitel die vielleicht ins falsche Lernfeld gehoeren. Hier sammeln statt
     stillschweigend aufloesen. -->

- _noch keine_
